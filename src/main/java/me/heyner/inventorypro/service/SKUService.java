...
public class SKUService {
  ...
  public SKU addSKU(UUID productUuid, SKUInputDto skuDto) throws EntityNotFoundException {
    Product product =
        productRepository
  ...
  public List<SKU> getSkus(UUID productUuid) throws EntityNotFoundException {
    List<SKU> skus = skuRepository.findByProduct_Id(productUuid);
    logger.info("SKUs {} found", skus.size());
    return skus;
  }

  public List<SKU> updateSkus(UUID productUUID, List<SKUInputDto> skuDtos)
    ...
    Product product =
        productRepository
            ...
            .setSkus(
                skuDtos.stream()
                    .map(skuDto -> new SKU().setSku(skuDto.getSku()))
                    .collect(Collectors.toList()));

    productRepository.save(product);

    return skuRepository.findByProduct_Id(productUUID);
  }
}
